...
import { importStudentsAction, ImportStudentRow } from './actions'
import { Upload } from 'lucide-react'

const COLUMNS = ['firstName', 'lastName', 'email', 'dateOfBirth', 'gender', 'yearLevel', 'enrollmentDate']
...
function validateRows(raw: Record<string, string>[]): CsvRow[] {
  return raw.map((data, i) => {
    ...
    if (data.yearLevel && isNaN(parseInt(data.yearLevel, 10))) {
      errors.push('yearLevel must be a number')
    }
    if (data.enrollmentDate && isNaN(Date.parse(data.enrollmentDate))) {
      errors.push('Invalid enrollmentDate (use YYYY-MM-DD)')
    }

    if (!data.dateOfBirth) warnings.push('No dateOfBirth')
    if (!data.yearLevel) warnings.push('No yearLevel')
...
export default function StudentImportClient() {
  ...
  function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = ev => {
      try {
        ...
        setFileError('Failed to parse CSV. Ensure the file is comma-separated.')
      }
    }
  ...
  function handleImport() {
    ...
    startTransition(async () => {
      const payload: ImportStudentRow[] = validRows.map(r => ({
        firstName: r.data.firstName,
        lastName: r.data.lastName,
        email: r.data.email,
        dateOfBirth: r.data.dateOfBirth || undefined,
        gender: r.data.gender || undefined,
        yearLevel: r.data.yearLevel || undefined,
        enrollmentDate: r.data.enrollmentDate || undefined,
      }))
      const res = await importStudentsAction(payload)
      setResult(res)
      if (res.imported > 0) {
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <p className="text-sm text-slate-600">
            Your CSV must have a header row with these columns (required columns marked with *):
          </p>
          <div className="font-mono text-xs bg-slate-50 border border-slate-200 rounded-md px-4 py-3 text-slate-700 overflow-x-auto whitespace-nowrap">
            firstName*, lastName*, email*, dateOfBirth, gender, yearLevel, enrollmentDate
          </div>
          <p className="text-xs text-slate-500">
            Dates should be in YYYY-MM-DD format. Students with an existing email will be skipped. Each imported student gets a random temporary password.
          </p>
        </CardContent>
      </Card>
          ...
          <CardContent className="space-y-4">
            ...
            {result ? (
              <div className="rounded-md bg-emerald-50 border border-emerald-200 px-4 py-3 text-sm text-emerald-700 space-y-1">
                ...
                <p>{result.imported} imported, {result.skipped} skipped (already exist)</p>
                {result.errors.map((e, i) => <p key={i} className="text-red-600">{e}</p>)}
                <p className="text-emerald-600">Redirecting to students list…</p>
              </div>
            ) : (
              <Button
